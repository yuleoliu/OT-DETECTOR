...
def get_test_labels(args, loader = None):
    ...
    elif args.in_dataset == "ImageNet_sketch":
        test_labels = obtain_ImageNet_classes()
    elif args.in_dataset == "ImageNet_A":
        test_labels = obtain_ImageNet_A_classes()
    elif args.in_dataset == "ImageNet_R":
        test_labels = obtain_ImageNet_R_classes()   
    elif args.in_dataset == "ImageNet_v2":
        test_labels = obtain_ImageNet_classes()
...
def obtain_ImageNet_A_classes():
    cls = open('datasets/ImageNet-A.txt').read().splitlines()

    cls = [x.split(' ',1)[-1].replace('_',' ') for x in cls]
    print('cls number',len(cls))

    return cls

def obtain_ImageNet_R_classes():
    cls = open('datasets/ImageNet-R.txt').read().splitlines()
    cls = [x.split(' ',1)[-1].replace('_',' ') for x in cls]
    print('cls number',len(cls))
    return cls    
